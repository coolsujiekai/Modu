...
项目使用 `cloudfunctions/quickstartFunctions` 获取 openid（用于客户端 `_openid` 过滤，提供安全防御纵深）。
...
**推荐索引**（减少 timeout，提升性能）：
- `books` 集合：复合索引 `(status, startTime)`、`(status, endTime)`、`(authorId, status)`、`_openid`
- `wishlist`、`authors`：`_openid` + 常用查询字段
